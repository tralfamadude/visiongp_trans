...
def main():
    ...
    with open(os.path.join(args.outputDirectory, "generations.csv"), 'w+') as generations_file:
        ...
        predicted_test_list = transPop.InputToPrediction(final_champion, test_filepath_list, interpreter, variableName_to_type, return_type)

        for i in range(0,len(predicted_test_list),1):
            image_path = os.path.join(args.outputDirectory, "image{}_{}.jpg".format(generationNdx, i))
            cv2.imwrite(predicted_test_list[i],image_path)
